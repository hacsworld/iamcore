...
async def bootstrap():
    ...
    uvicorn.run(app, host="127.0.0.1", port=8000, log_level="info")⸻ core/Dockerfile  # с Grok tweaks: healthcheck + deps cleanupFROM python:3.12-slim  # Grok: bumped to 3.12 for perfENV PYTHONDONTWRITEBYTECODE=1 
    PYTHONUNBUFFERED=1RUN apt-get update && apt-get install -y --no-install-recommends 
    ffmpeg tesseract-ocr libgl1-mesa-glx libglib2.0-0 && \  # Grok: added libglib for cv2
    rm -rf /var/lib/apt/lists/*WORKDIR /app
COPY requirements.txt ./
RUN pip install --no-cache-dir -r requirements.txtCOPY . .EXPOSE 8000
HEALTHCHECK --interval=30s --timeout=5s --retries=3 
    CMD curl -fsS -H "X-API-Key: ${API_KEY}" http://127.0.0.1:8000/health || exit 1  # Grok: addedCMD ["uvicorn", "app:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "2"]  # Grok: uvicorn direct for workers⸻ docker-compose.yml  # с Grok tweaks: networks, readonly volumes, depends_onversion: "3.9"
services:
  hacs-core-blue:
    build: ./core
    container_name: hacs-core-blue
    env_file:
      - ./core/.env
    environment:
      - UVICORN_WORKERS=2
    ports:
      - "8000:8000"
    healthcheck:
      test: ["CMD-SHELL", "curl -fsS -H 'X-API-Key: ${API_KEY}' http://127.0.0.1:8000/health || exit 1"]
      interval: 15s
      timeout: 5s
      retries: 5
    ...
    volumes:
      ...
      - ./data/vault:/app/vault:ro  # Grok: ro for vault
    networks:
      - hacs-net  # Grok: isolated network  hacs-core-green:
    build: ./core
    container_name: hacs-core-green
    env_file:
      - ./core/.env
    environment:
      - UVICORN_WORKERS=2
    ...
    volumes:
      - ./data/state:/app/state
      - ./data/outputs:/app/outputs
      - ./data/vault:/app/vault:ro
    networks:
      - hacs-net
    depends_on:
      hacs-core-blue:
        condition: service_healthy  # Grok: green waits blue healthynetworks:
  hacs-net:  # Grok: custom network
    driver: bridge⸻ core/.env  # твой прод-шаблон без измененийcore/.env (prod)API_KEY=super-secret-long
ALLOWLIST=wikipedia.org,docs.python.org,ffmpeg.org,arxiv.org,github.com,x.ai
USE_GENERATION=grok            # none|ollama|grok
GROK_API_KEY=sk-live-XXXXXXXX
GROK_MODEL=grok-4-fastOLLAMA_MODEL=llama3.1:8b
AUTOSAVE_SEC=30
MEMORY_PATH=./state/memory.jsonHUMOR_MODE=friendly            # off|friendly|spicy
REGION=ru-RU
HUMOR_USER_NAME=бро
