...
app = FastAPI(title="Resonance Core (HACS)", version="3.4-prod")
...
CLOUD = CloudGateEssence(embedder=lambda x: model.encode(x), allow_domains=CLOUD_ALLOW, timeout_s=10.0)
...
async def setup_portfolio(
    ...
    return {"status":"OK","message":"Portfolio seeded","tags":tags,"cloud_suggested":bool(use_cloud)}
    ...
    for base in AUTO_PATHS:
        ...
        for f in base.rglob("*"):
            ...
            try:
                ...
                continue
    return {"status":"OK","scanned_files":scanned,"ingested_chunks":ing}
...
async def ingest_url(url: str = Form(...), tag: str = Form(default="generic"), dep: bool = Depends(require_key)):
    ...
    for i in range(0, len(raw), step):
        ...
        ing += 1
        if i > 1500*20: break
    return {"status":"OK","kind":kind,"filename":name,"ingested_chunks":ing,"approx_chars":len(raw)}
...
def maybe_humorize(text: str, req: ChatRequest) -> str:
...
async def cloud_accelerate(req: CloudReq, dep: bool = Depends(require_key)):
    ...
    for d in res["distilled"]:
        ...
        ing += 1
    return {"status":"OK","ingested":ing,"sources":[{"url":d["url"],"title":d["title"]} for d in res["distilled"]]}

# ---------- Vault ----------
class VaultExportReq(BaseModel):
    passphrase: str
...
async def ui_chat():
    return """
<!doctype html><html><head><meta charset="utf-8"><title>Resonance Chat</title></head>
<body style="font-family:system-ui;margin:24px;max-width:840px">
<h2>Resonance Chat (3.4 Prod)</h2>
